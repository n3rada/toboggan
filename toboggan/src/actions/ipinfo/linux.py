...
class IpInfoAction(BaseAction):
    ...
    def run(self) -> str:
        ...
        ls_result = self._executor.remote_execute("ls -1 /sys/class/net", retry=False)
        if not ls_result:
        ...
        for iface_name in interfaces:
            ...
            idx_result = self._executor.remote_execute(f"cat /sys/class/net/{iface_name}/ifindex 2>/dev/null || echo '0'", retry=False)
            idx = idx_result.strip() if idx_result else "0"

            # Read interface state
            state_result = self._executor.remote_execute(f"cat /sys/class/net/{iface_name}/operstate 2>/dev/null || echo 'unknown'", retry=False)
            ...
            # Read MTU
            mtu_result = self._executor.remote_execute(f"cat /sys/class/net/{iface_name}/mtu 2>/dev/null || echo '1500'", retry=False)
            ...
            # Read MAC address
            mac_result = self._executor.remote_execute(f"cat /sys/class/net/{iface_name}/address 2>/dev/null", retry=False)
            ...
            # For per-interface mapping, we'd need ip/ifconfig commands which we're avoiding
            
            # Alternative: Read /proc/net/route to find IPs associated with this interface
            route_data = self._executor.remote_execute("cat /proc/net/route 2>/dev/null", retry=False)
            if route_data:
            ...
            ipv6_result = self._executor.remote_execute(f"grep '{iface_name}' /proc/net/if_inet6 2>/dev/null", retry=False)
            if ipv6_result:
                ipv6_count = 0
                ...
                if ipv6_count > 0:
                    logger.debug(f"  🌍 Found {ipv6_count} IPv6 address(es)")
        ...
        proc_route = self._executor.remote_execute("cat /proc/net/route 2>/dev/null", retry=False)
        if proc_route and proc_route.strip():
            ...
            if routes:
                ...
                logger.success(f"✅ Parsed {len(routes)} route(s) from routing table")
        else:
            output_lines.append("⚠️  Could not read routing table from /proc/net/route")
            logger.warning("⚠️ Could not read routing table")

        logger.success(f"🎉 Successfully retrieved information for {len(interfaces)} interface(s) and routing table")
